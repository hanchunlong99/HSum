...
class Translator(object):
    ...
    def translate(self,
        ...
        self.model.eval()
        gold_path = self.args.result_path + 'cnndm.%d.gold' % step
        can_path = self.args.result_path + 'cnndm.%d.candidate' % step
        ...
        raw_src_path = self.args.result_path + 'cnndm.%d.raw_src' % step
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for trans in translations:
                    pred, gold, src = trans
                    pred_str = pred.replace('<s>', '').replace('<pad>', '').replace('</s>', '').replace(r' +', ' ').strip()
                    gold_str = gold.strip()
                    if(self.args.recall_eval):
                        _pred_str = ''
                        gap = 1e3
    ...
    def _fast_translate_batch(self,
        ...
        src_features = self.model.xlnet(src, segs, mask_src+0)[0]
