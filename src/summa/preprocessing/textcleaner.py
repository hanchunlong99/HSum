...
def replace_with_separator(text, separator, regexs):
    ...
    result = text
    for regex in regexs:
        result = regex.sub(replacement, result)  # replacement是被替换成的文本, result是需要被替换的文本
    return result
...
def filter_words(sentences):
    ...
    #            stem_sentence]  # 句子 转小写，去数字 ，去标点符号 ， 去停用词， 单词转换词干

    # filters = [lambda x: x.lower(),  stem_sentence]
...
def clean_text_by_sentences(text_list):

    filtered_sentences = filter_words(text_list)

    return merge_syntactic_units(text_list, filtered_sentences)
